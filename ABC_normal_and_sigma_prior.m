...
accepted_sigma_array_no_nan] = ABC_normal_and_sigma_prior()
...
number_of_parameters_to_estimate = 8;
...
tolerance = 0.05*total_number_of_sampled_times;
...
accepted_sigma_array = nan(number_of_iterations,1);
...
mean_V0_sum = 0;
standard_deviation_V0_sum = 0.10;
...
number_of_accepted_parameters = 0;
...
%     V0_sum = lognrnd(mean_V0_sum, standard_deviation_V0_sum);
    ...
    for experiment_with_certain_ratio=1:number_of_different_ratios_tried
        ...
        V_simulated_1 = transpose(V_simulated(:,1)) + sigma*...
            randn(1,length(V_simulated(:,1)));
        V_simulated_2 = transpose(V_simulated(:,2)) + sigma*...
...
%         sum_squared_errors_sum;
    ...
    if (total_squared_errors <= tolerance)
        plot(sampled_times_sum_data,V_sum_data_average_PC3_0Gy(experiment_with_certain_ratio,:),...
            'r');
        hold on
        plot(sampled_times_sum_data,V_simulated_sum,'b--');
        title('Accepted');
        
        number_of_accepted_parameters = number_of_accepted_parameters + 1;
        
        if number_of_accepted_parameters == 100
            drawnow;
        end
        ...
        accepted_sigma_array(iteration) = sigma;
    else
%         plot(sampled_times_sum_data,V_simulated_sum,'b-');
%         hold on
%         plot(sampled_times_sum_data,V_sum_data_average_PC3_0Gy(experiment_with_certain_ratio,:),...
%             'r');
%         legend({'Simulated sum','True sum'});
%         title('Rejected');
%         hold off
    end
end
...
display(['sigma = ', num2str(approximate_posterior_mean_sigma)]);
...
display(['sigma = ', num2str(approximate_posterior_median_sigma)]);
...
display(['sigma = ', num2str(MAP_estimate_sigma)]);
...
display(['For sigma: ', num2str(number_of_accepted_sigma)]);
...
function parameter = get_positive_parameter_sampled_from_normal_distribution(...
    ...
    while parameter_negative
    parameter = normrnd(mean,standard_deviation,1,sample_size);
